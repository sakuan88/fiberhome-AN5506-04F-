...
function showDate(wanIndex)
{
	...
	/* add by 吴小娟, 20180719, 原因:巴西TIM普通用户不得修改INTERNET WAN的业务类型 */
	if(ispName == 26 && curUserType == 1 && curWANmode == 1)
	{		
		document.getElementById("wan_connectionmode").disabled = 1;
	}
...
//	if( ispName == 15 )//GLOBE
	...
	//16:TR069_VOIP 17: TR069_VOIP_INTERNET  7:RADIUS(同VOIP) 8:RADIUS_INTERNET
	if ((chooseWanValue == 0) ||	(chooseWanValue == 1) 
		|| (chooseWanValue == 2) || (chooseWanValue == 4) || (chooseWanValue == 5)
		|| (chooseWanValue == 7)	//RADIUS(同VOIP)
...
*/
		...
		//TR069 VOIP相关 
		if(chooseWanValue == 0 || (chooseWanValue == 4) || (chooseWanValue == 7)
		...
		{				
				...
				if(chooseWanValue == 0 || (chooseWanValue == 4)
					|| (chooseWanValue == 7) || (chooseWanValue == 16))//tr069、voip、tr069_voip、RADIUS
				{
					setDisplay("port_bang", "none");
					setDisplay("static_ipv6", "none");
					setDisplay("dhcp_ipv6", "none");
...
{ 
	...
	if(ispName == 12 || ispName == 28 || ispName == 29)/* 厄瓜多尔HGU/墨西哥TOTALPLAY /智利X_ENTEL*/
	{
		Have_RADIUS = 1;
	}
	...
	if(ispName == 28 || ispName == 29)/* 墨西哥TOTALPLAYHGU/智利X_ENTEL*/
	{
		wan_connectionmodeNode.options[wan_connectionmodeNode.length] = new Option("IPTV", "6");
	}
	if((ispName == 26) && (curUserType == 1))/* 巴西TIM 普通用户不显示DNS Relay 配置项*/
	{
		setDisplay("tr_dnsrelay", "none");
	}
	else
	{
		setDisplay("tr_dnsrelay", "");
	}
	...
	{	
		var row0 = getElement("fw_ruletable").rows[2];
		var firstTrID = row0.getAttribute("id");
	...
	//add by wuxj, 20160809, ROMANIA add pppAuthType etc.
	if((ispName == 12) || (ispName == 9) || (ispName == 16))/*厄瓜多尔 罗马尼亚 越南 hgu*/
	{
		setDisplay("tr_pppAuthType", ""); 
		setDisplay("tr_pppConnTrigger", "");
	}
...
function clickAdd(tabTitle)
{
	...
	if(ispName == 26 && curUserType == 1)	/* add by 吴小娟, 20180719, 原因:巴西TIM普通用户不得新增 */
	{
		alert("You don't have this permission for now!");
		return ;
	}
...
//	if((ispName == 5) || (ispName == 9) || (ispName == 28) || (ispName == 29))//AIS/罗马尼亚/墨西哥TOTALPLAY/智利X_ENTEL vid可以一样
	...
	{		
		...
		if((ispName == '28') || (ispName == '29'))//墨西哥TOTALPLAY/智利X_ENTEL
		{
