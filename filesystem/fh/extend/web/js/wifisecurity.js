...
function parseAllData(str)
{
	...
	//add by wuxj, 20180913, 增加支持非英文语言字符
	var ssidArr = new Array();
	ssidArr = ssid_arr.split(";");//;间隔,类似;0;0;0;0
	
	if((ispNameCode == 9 || ispNameCode == 22) && (curUserType == 1)) /*罗马尼亚/泰国TRUE版本普通用户,只显示ssid1*/
	{
...
//		ssidName[i] = fields_str[24];
		ssidHide[i] = fields_str[24];
/* add end by 吴小娟, 20111018 */

		/* add by 吴小娟, 20160811, 原因: 增加支持WMM */
		wmmList[i] = fields_str[25];
		if(ispNameCode == '26')//巴西TIM
			guestList[i] = fields_str[26];//add by wuxj, 20180319
...
/* modify end by 吴小娟, 20110920 */
		...
		//add by wuxj, 20180914, add check ASCII
		if(ispNameCode == 26)//BRAZIL_TIM
		{
			if(checkASCIIChar(document.security_form.passphrase.value) == false){
				//alert('Invalid characters in Pass Phrase.');
				alert(_("secure invalid character in pass"));
				document.security_form.passphrase.focus();
				return false;
	...
	var keylength = document.security_form.WEP1.value.length;
		...
		if (document.security_form.WEP1Select.options.selectedIndex == 0){
			if(keylength != 5 && keylength != 13) {
				...
				document.security_form.WEP1.focus();
				return false;
			}
			...
			if(ispNameCode == 26)//BRAZIL_TIM
			{
				if(checkASCIIChar(document.security_form.WEP1.value)== false){
					alert("WEP key1" + _(" contains invalid characters."));
					document.security_form.WEP1.focus();
					return false;
				}
	...
	keylength = document.security_form.WEP2.value.length;
		if (document.security_form.WEP2Select.options.selectedIndex == 0){
			if(keylength != 5 && keylength != 13) {
				...
				document.security_form.WEP2.focus();
				return false;
			}
			...
			if(ispNameCode == 26)//BRAZIL_TIM
			{	
				if(checkASCIIChar(document.security_form.WEP2.value)== false){
					alert("WEP key2" + _(" contains invalid characters."));
					document.security_form.WEP2.focus();
					return false;
				}
	...
	keylength = document.security_form.WEP3.value.length;
		if (document.security_form.WEP3Select.options.selectedIndex == 0){
			if(keylength != 5 && keylength != 13) {
				...
				document.security_form.WEP3.focus();
				return false;
			}
			...
			if(ispNameCode == 26)//BRAZIL_TIM
			{
				if(checkASCIIChar(document.security_form.WEP3.value)== false){
					alert("WEP key3" + _(" contains invalid characters."));
					document.security_form.WEP3.focus();
					return false;
				}
	...
	keylength = document.security_form.WEP4.value.length;
		if (document.security_form.WEP4Select.options.selectedIndex == 0){
			if(keylength != 5 && keylength != 13) {
				...
				document.security_form.WEP4.focus();
				return false;
			}
			...
			if(ispNameCode == 26)//BRAZIL_TIM
			{
				if(checkASCIIChar(document.security_form.WEP4.value)== false){
					alert("WEP key4" + _(" contains invalid characters."));
					document.security_form.WEP4.focus();
					return false;
				}
...
function LoadFields(MBSSID)
{
	...
	/*印尼电信版本/厄瓜多尔HGU/墨西哥TOTALPLAY*/
	if(ispNameCode == 13 || ispNameCode == 12 || ispNameCode == 28)
	{
		Have_RADIUS = 1
	}
...
/*	if(AuthMode[MBSSID].toString().toUpperCase() == "OPEN" && EncrypType[MBSSID].toString().toUpperCase() == "NONE")*/
	...
	//WIFI联盟测试、巴西TIM不需要tkip
	if(WEB_WIFI_CERTIFIED_OPTION == '1' || ispNameCode == 26)
	{
...
function selectMBSSIDChanged()
{
	...
	/* 安徽移动普通用户对SSID2-4可见不可配 、黑龙江移动普通用户和管理用户SSID2-4可见不可配 */
