...
*/	
	...
	if(WEB_COMM_WAN_CFG == 1)
	{
		ipv6_enable_s.value = ipv6_state_arr[wanIndex];
		dslite_enable_s.value = dslite_enable_arr[wanIndex];
		aftr_mode_s.value = aftr_mode_arr[wanIndex];
		AFTR_value_node.value = aftr_value_arr[wanIndex];
		
		staticIp_ipv6_node.value = space2null(staticIp_ipv6_arr[wanIndex]);
		IPv6_Gateway_node.value = space2null(IPv6_Gateway_arr[wanIndex]);
		IPv6_Pri_DNS_node.value = space2null(IPv6_Pri_DNS_arr[wanIndex]);
		IPv6_Sec_DNS_node.value = space2null(IPv6_Sec_DNS_arr[wanIndex]);
		Prefix_Length_node.value = space2null(Prefix_Length_arr[wanIndex]);
		IPv6_prefix_node.value = space2null(IPv6_prefix_arr[wanIndex]);
	}
	else
	{
		...
		AFTR_value_node.value = aftr_value;

		staticIp_ipv6_node.value = staticIp_ipv6;
		IPv6_Gateway_node.value = IPv6_Gateway;
		IPv6_Pri_DNS_node.value = IPv6_Pri_DNS;
		IPv6_Sec_DNS_node.value = IPv6_Sec_DNS;
		Prefix_Length_node.value = Prefix_Length;
		IPv6_prefix_node.value = IPv6_prefix;
	}
...
//	ipModelSwitch();	
	...
	{
		document.getElementById("staticIp").value = space2null(curWANstaticIP);
		document.getElementById("staticNetmask").value = space2null(curWANstaticMask);
		document.getElementById("staticGateway").value = space2null(curWANstaticGW);
		document.getElementById("staticPriDns").value = space2null(curWANstaticDns1);
		document.getElementById("staticSecDns").value = space2null(curWANstaticDns2);
	}
