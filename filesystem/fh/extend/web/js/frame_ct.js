...
var Frame = {
    ...
    initData : function() {
        ...
        this.mainMenuCounter = 0;//一级菜单索引
        this.subMenuCounter = 0;//二级菜单索引

        this.$mainMenu = $("#headerTab ul");
        this.$subMenu = $("#topNav ul");
...
//    	$("#headerTitle").text(productName);
        ...
        $("#topNav ul li.subMenu:eq(0)").click();
    },
    /**
     * 添加菜单项
     * @param {} element
    ...
    addMenuItems : function(element, menus, type) {
        ...
        if (type == "main") {
            ...
            for (var i = 0, len = menus.length; i < len; i++) {				
				element.append('<li value="' + i + '">' + _(menus[i].name) + '</li>');
            }
            ...
            element.children("li").click(function() {
                if (!element.children("li").eq(this.value).is(".selected")) {
                    ...
                    frame.addMenuItems($("#topNav ul"), menus[this.value].subMenus, "sub");
            ...
            element.find("li").click(function() {
                element.find("li").eq(frame.subMenuCounter).removeClass("selected");
                $(this).addClass("selected");
                frame.subMenuCounter = this.value;
                frame.addMenuItems($("#navsub"), menus[this.value].subMenus, "sub2");
