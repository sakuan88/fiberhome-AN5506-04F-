...
function initTranslation()
{
	...
	/*add begin by 陈鹏 20180911 原因:墨西哥版本要求增加开发协议，需求来源:肖保平*/
	if(ispNameCode == 28)
	{
		e = document.getElementById("pf_openprotocolTitle");
		e.innerHTML = _("pf_openprotocolTitle");

		e = document.getElementById("The_OpenProtocolt");
		e.innerHTML = _("pf_openprotocolTitle");
	}
	
	/*add end by 陈鹏 20180911*/
...
{ 
	...
	if(ispNameCode == 28)
	{	
		var pt_TriprotocolNode= document.getElementById("pt_Triprotocol");
		pt_TriprotocolNode.options.remove(0);
		pt_TriprotocolNode.options[pt_TriprotocolNode.length] = new Option("TCP/UDP", "4");
		
		setDisplay("The_OpenProtocolt", "" );
		setDisplay("tr_openprotocol", "" );
		
		document.getElementById("The_trigger_Port").width = "13%";
		document.getElementById("The_open_Port").width = "13%";
		document.getElementById("The_TriggerProtocol").width = "17%";
		document.getElementById("The_OpenProtocolt").width = "14%";
	}
...
function selectLine(id)
{
	...
	if (objTR != null)
	{
		...
		{
			document.getElementById("pt_Description").value = document.getElementById("pt_Description_" + temp).innerHTML;
			...
			/*modify begin by 陈鹏 20180911 原因:墨西哥版本要求增加开发协议，需求来源:肖保平*/
			if(ispNameCode == 28)
			{
				var Openprotocol_value = document.getElementById("pt_Openprotocol_" + temp).innerHTML;
				var pt_Openprotocol_ID = document.getElementById("pt_openprotocol");
				...
				else if(Triprotocol_value == "TCP/UDP")
				{
					pt_Triprotocol_ID.value = "4";
				}			

				if( Openprotocol_value == "TCP")
				{
					pt_Openprotocol_ID.value = "1";
				}
				else if(Openprotocol_value == "UDP")
				{
					pt_Openprotocol_ID.value = "2";
				}
				else if(Openprotocol_value == "TCP/UDP")
				{
					pt_Openprotocol_ID.value = "4";
				}
			}
			else
			{
...
//	if(confirm("你确认删除所选规则?") == false)
	...
	document.location = "/goform/portTriggerDeleteAll";
          ...
          <table class="tabal_bg" id = "fw_ruletable" border="0" cellpadding="0" cellspacing="1" width="100%">
            <tbody>
              ...
              <tr class="tabal_title">
                ...
                <td width="13%" align="center" id="The_OpenProtocolt" display="none">Open Protocol</td>
                          ...
                          <td ><div id="div_pf_rule">
                              <table class="tabal_bg" cellpadding="0" cellspacing="1" width="100%">
                                <tbody>
                                  ...
                                  <!--墨西哥版本要求增加开发协议，需求来源:肖保平-->
                                  <tr style="" id="tr_openprotocol" display="none">
                                    <td id="pf_openprotocolTitle" class="tabal_left" width="25%">Open Protocol</td>
                                    <td class="tabal_right" width="75%"><select name="pt_openprotocol" id="pt_openprotocol" size="1" style="width:14%">
                                        <option value="1">TCP</option>
                                        <option value="2">UDP</option>
                                        <option value="4" id="open_protocol_opt"selected="selected">TCP/UDP</option>
                                      </select></td>
                                  </tr>
                                  <tr style="">
