...
var wifiPowerCtrlSync = '<% wifiPowerCtrlSync(); %>';
var WifiConNumSync = '<% WifiConNumSync(); %>';
...
function initValue()
{
	...
	else if(ispNameCode == '26')	//add by wuxj, 20180103, for TIM
	{
	...
	if(ispNameCode == 9 && curUserType == 1) /*罗马尼亚版本普通用户*/
	{
		setDisplay("wifi_power_table", "none");
		setDisplay("wifi_power_apply_table", "none");
...
<form method=post name="wifi_power" action="/goform/setWifiPower">
  ...
  <table id = "wifi_power_table" class="tabal_bg" border="0" cellpadding="0" cellspacing="1" width="100%">
    <tbody>
      <tr>
  ...
  <table id = "wifi_power_apply_table" class="tabal_button" border="0" cellpadding="0" cellspacing="1" width="100%">
    <tbody>
      <tr>
...
<form method=post name="wifi_power" action="/goform/setWifiConNum" onSubmit="return CheckValue()">
