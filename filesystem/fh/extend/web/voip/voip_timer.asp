...
if(ispNameCode == 15)   //菲律宾GLOBE加载voip_globe.xml
{
...
function LoadFrame()
{
	...
	/* add begin by 陈鹏，20180522 原因:马电 TM 新增SIP定时器 */
	if(ispNameCode == '21')
	{
		setDisplay("tr_timerenable_title", "");
		var get_timer_enable = "<% getCfgGeneral(1, "sip_timer_enable"); %>";
		var timer_enable = document.getElementById("sip_timer_enable");
		showSelectNodeByValue(timer_enable, get_timer_enable);
		connectionTypeSwitch();		
	}
	/* add begin by 陈鹏，20180522 */
...
{
	var timer_enable = document.getElementById("sip_timer_enable");
	var voip_sip_sessiontimer = document.getElementById("voipsipsessiontimer");
	if (timer_enable.options.selectedIndex == 0) 
	{
		setDisplay("tr_sipsessiontimer_title", "");
		voip_sip_sessiontimer.value = "<% getCfgGeneral(1, "voipsipsessiontimer"); %>";
	}
	else
	{
		setDisplay("tr_sipsessiontimer_title", "none");	
	}
}
...
function CheckTimeForm()
{
	...
	if(ispNameCode == '21')		//马来电信 X_TM
	{
		/* validate voipsipsessiontimer"); */
		var timer_enable = document.getElementById("sip_timer_enable");
		field = document.getElementById("voipsipsessiontimer");
		field.value = trim(field.value);
		if(timer_enable.options.selectedIndex == 0)	/*定时器使能时对时间值进行输入校验，索引0为使能*/
		{
			if (!CheckNotNull(field.value)) 
			{					/* not null */
				alert("Please input voip sip session timer.");
				field.value = field.defaultValue;
				field.focus();
				return false;
			}
			else if(!checkNumberLegal(field, 90, 65535))					/* validate whether legal */
			{		
				field.value = field.defaultValue;
				field.focus();
				alert("The voipsipsessiontimer you input is illegal");
				return false;
			}
		}
	}
...
<form method=post name="fm_voip_timer" action="/goform/setVoipTimer" onSubmit="return CheckTimeForm()">
  ...
  <table class="tabal_bg" border="0" cellpadding="0" cellspacing="1" width="100%">
    <tbody>
      ...
      <tr id="tr_timerenable_title"  style="display: none">
        <td id="timerenable_title" class="tabal_left" width="25%">SIP Session Timer Enable</td>
        <td class="tabal_right" align="left"><select name="sip_timer_enable" id="sip_timer_enable" size="1" style="width:200px;" onChange="connectionTypeSwitch();">
			<option id="timer_enable" value="1" selected="selected">Enable</option>
            <option id="timer_disable" value="0" >Disable</option>
          </select></td>
      </tr>
      <tr id="tr_sipsessiontimer_title" style="display: none">
        <td class="tabal_left" id="sipsessiontimer_title" align="left" width="25%">SIP Session Timer</td>
        <td class="tabal_right" align="left"><input type="text" name="voipsipsessiontimer" id="voipsipsessiontimer" size="30" maxlength="5">
          &nbsp; &nbsp; 90~65535(s) </td>
      </tr>
