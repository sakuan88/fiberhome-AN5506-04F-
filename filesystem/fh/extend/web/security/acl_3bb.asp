...
{ 
	...
	var acl_apptypeNode = getElement("acl_apptype");
	var acl_directionNode = getElement("acl_direction");
	if(ispNameCode == '12' || ispNameCode == '28' || ispNameCode == '29' )//厄瓜多尔HGU/墨西哥TOTALPLAY/智利X_ENTEL
	{
		acl_apptypeNode.options[acl_apptypeNode.length] = new Option('SSH', 6);
	}
	...
	else if(ispNameCode == '21')//HGU TM
	{
 		//modify by wuxj, 20180622, descrip:support lan/wan,
									//support ICMP、HTTP、HTTPS、FTP、TELNET/SSH 需求来源:钱凯
		acl_apptypeNode.options[acl_apptypeNode.length] = new Option('HTTPS', 9);
		acl_apptypeNode.options.remove(4);	//SNMP
		acl_apptypeNode.options[2].text = "Telnet/SSH";	//TELNET
	}
...
function clickRemoveAll(tabTitle)
{
	...
	document.location = "/goform/ACL_3bbDeleteAll";	
}
