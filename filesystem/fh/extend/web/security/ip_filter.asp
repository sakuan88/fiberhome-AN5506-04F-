...
var previousTR = null;
var ipFilterSync = '<% ipFilterSync(); %>';

var fw_rulesSum = '<% getCfgGeneral(1, "fw_rulesSum"); %>';

var fw_directionList = '<% getCfgGeneral(1, "fw_ruleDirectionList"); %>';
var directionArray = fw_directionList.split("|");

var fw_srcIPStartList = '<% getCfgGeneral(1, "fw_ruleSrcIPStartList"); %>';
var srcIPStartArray = fw_srcIPStartList.split("|");
var fw_srcIPEndList = '<% getCfgGeneral(1, "fw_ruleSrcIPEndList"); %>';
var srcIPEndArray = fw_srcIPEndList.split("|");
var fw_srcPortStartList = '<% getCfgGeneral(1, "fw_ruleSrcPortStartList"); %>';
var srcPortStartArray = fw_srcPortStartList.split("|");
var fw_srcPortEndList = '<% getCfgGeneral(1, "fw_ruleSrcPortEndList"); %>';
var srcPortEndArray = fw_srcPortEndList.split("|");

var fw_dstIPStartList = '<% getCfgGeneral(1, "fw_ruleDstIPStartList"); %>';
var dstIPStartArray = fw_dstIPStartList.split("|");
var fw_dstIPEndList = '<% getCfgGeneral(1, "fw_ruleDstIPEndList"); %>';
var dstIPEndArray = fw_dstIPEndList.split("|");
var fw_dstPortStartList = '<% getCfgGeneral(1, "fw_ruleDstPortStartList"); %>';
var dstPortStartArray = fw_dstPortStartList.split("|");
var fw_dstPortEndList = '<% getCfgGeneral(1, "fw_ruleDstPortEndList"); %>';
var dstPortEndArray = fw_dstPortEndList.split("|");

var fw_protocolList = '<% getCfgGeneral(1, "fw_ruleProtocolList"); %>';
var protocolArray = fw_protocolList.split("|");

function fw_checkIP(dom)
{	
	if(dom.style.display == "none")//结束IP
	{
		return true;
	}
	dom.value = trim(dom.value);
	if(!CheckNotNull(dom.value))	//允许为空
	{
		return true;
	}
...
//		getElement("fw_dstPortEnd").value = "0";
	...
	{	
		var index = getElement("fw_curIndex").value;
		...
		if(index != "new" && index != "no")
		{
			index = parseInt(getElement("fw_curIndex").value);
			getElement("fw_srcPortStart").value = srcPortStartArray[index];
			getElement("fw_srcPortEnd").value = srcPortEndArray[index];
			getElement("fw_dstPortStart").value = dstPortStartArray[index];
			getElement("fw_dstPortEnd").value = dstPortEndArray[index];
		}		
	}
}
...
//	if(confirm("你确认删除所选规则?") == false)
	...
	document.location = "/goform/ipFilterDeleteAll";	
}

function checkValue()
{
	if(!fw_checkIP(document.getElementById("fw_srcIPStart")))
	{
		return false;
	}
	if(!fw_checkIP(document.getElementById("fw_srcIPEnd")))
	{
		return false;
	}
	if(!fw_checkIP(document.getElementById("fw_dstIPStart")))
	{
		return false;
	}
	if(!fw_checkIP(document.getElementById("fw_dstIPEnd")))
	{
		return false;
	}
...
//			getElement("fw_curWanIP").innerHTML = _("ipFilter_curWanIP") + devWanIPArray[0];
		...
		else
		{
			...
			document.getElementById("fw_direction").value = directionArray[index];
			document.getElementById("fw_srcIPStart").value = srcIPStartArray[index];
			document.getElementById("fw_srcIPEnd").value = srcIPEndArray[index];
			document.getElementById("fw_srcPortStart").value = srcPortStartArray[index];
			document.getElementById("fw_srcPortEnd").value = srcPortEndArray[index];
			document.getElementById("fw_dstIPStart").value = dstIPStartArray[index];
			document.getElementById("fw_dstIPEnd").value = dstIPEndArray[index];
			document.getElementById("fw_dstPortStart").value = dstPortStartArray[index];
			document.getElementById("fw_dstPortEnd").value = dstPortEndArray[index];
			document.getElementById("fw_protocol").value = protocolArray[index];
/*
...
//	e.value = _("firewall_cancel");
	...
	e.innerHTML = _("firewall_ipv4filterup_title");
...
/* add begin by 吕轶超, 20120227, 原因: */
	var upnable = document.getElementsByName("ipfilter_up");	
	for(var i = 0;i<upnable.length;i++)
	{
		if(upnable[i].value == '<% getCfgGeneral(1, "ipfilter_up"); %>')
		{
...
<form method="post" id="fw_white_enable" action="/goform/Black_White_Cfg">
  <table class="tabal_bg" border="0" cellpadding="0" cellspacing="1" width="100%">
    <tbody>
      <tr>
        <td class="tabal_left" id="ipfilterup_title" align="left" width="25%">Filter Mode</td>
        <td class="tabal_right" colspan="6" align="left" width="75%"><input checked="checked" value="white" name="ipfilter_up" type="radio">
          <span id="up_white">White List</span> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
          <input value="black" name="ipfilter_up" type="radio">
...
<table border="0" cellpadding="0" cellspacing="0" height="5" width="100%">
  <tbody>
    ...
    <tr>
      <td id="fw_ruleList"><form method="post" id="fw_ruleForm" action="/goform/ipFilterDelete">
...
//				document.write('<TD align="center">' + dstIPStartArray[i] + '--'+ dstIPEndArray[i] + '</TD>');
    ...
    <tr>
      <td><form id="ConfigForm" method="post" action="/goform/ipFilterCfg" onSubmit="return checkValue()">
          <div id="ConfigForm1">
                          ...
                          <td ><div id="div_fw_rule">
                              <table class="tabal_bg" cellpadding="0" cellspacing="1" width="100%">
                                <tbody>
                                  ...
                                  <tr style="">
                                    <td id="fw_srcIPTitle2" class="tabal_left" width="25%">Src IP</td>
                                    <td class="tabal_right" width="75%"><input name="fw_srcIPStart" id="fw_srcIPStart" style="width: 150px;" type="text">
                                      &nbsp;&nbsp;-&nbsp;&nbsp;
                                      <input name="fw_srcIPEnd" id="fw_srcIPEnd" style="width: 150px;" type="text"></td>
                                  </tr>
                                  <tr style="">
                                  ...
                                  <tr style="">
                                    <td id="fw_dstIPTitle2" class="tabal_left" width="25%">Dst IP</td>
                                    <td class="tabal_right" width="75%"><input name="fw_dstIPStart" id="fw_dstIPStart" style="width: 150px;" type="text">
                                      &nbsp;&nbsp;-&nbsp;&nbsp;
                                      <input name="fw_dstIPEnd" id="fw_dstIPEnd" style="width: 150px;" type="text">
