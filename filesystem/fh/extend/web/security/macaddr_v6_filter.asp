...
var pf_ruleSum;
var macaddr_filter_enableSync = '<% macaddr_v6_filter_enableSync(); %>';
var macaddr_getsizeSync = '<% macaddr_v6_getsizeSync(); %>';

function initTranslation()
{
	var e = document.getElementById("macaddr_filterPrompt");
	e.innerHTML = _("v6_macaddr_filterPrompt");

	e = document.getElementById("macaddrenable_title");
	e.innerHTML = _("v6_macaddrenable_title");
	...
	e.innerHTML = _("v6_macaddrdown_title");
	...
	e.innerHTML = _("v6_macaddr_filterListHead");
...
{ 
	...
	{
		if(macAddrUp[i].value == '<% getCfgGeneral(1, "mac_v6_black_white"); %>')
		{
	...
	{
		if(macfilterable[i].value == '<% getCfgGeneral(1, "macaddr_v6_filter_enable"); %>')
		{
	...
	pf_ruleSum  = '<% getCfgGeneral(1, "macaddrV6_size"); %>';
...
function selectLine(id)
{
	...
	if (objTR != null)
	{
		...
		{
			document.getElementById("macaddr_mac").value = document.getElementById("mac_adress_" + temp).innerHTML;
...
//	if(confirm("你确认删除所选规则?") == false)
	...
	document.location = "/goform/macaddr_v6_filterDeleteAll";
...
<form method="post" id="macaddr_enable" action="/goform/macaddr_v6_filter_enableCfg">
...
<table border="0" cellpadding="0" cellspacing="0" height="5" width="100%">
  <tbody>
    ...
    <tr>
      <td id="mf_ruleList"><form method="post" id="mf_ruleForm" action="/goform/macaddr_v6_filterDelete">
    ...
    <tr>
      <td><form id="ConfigForm" method="post" action="/goform/macaddr_v6_filterCfg" onSubmit="return checkValue()">
          <div id="ConfigForm1">
