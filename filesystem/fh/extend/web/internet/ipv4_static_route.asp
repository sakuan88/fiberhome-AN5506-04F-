...
function selectLine(id)
{
	...
	if (objTR != null)
	{
		...
		if (temp == 'new')		//新建
		{
			getElement("v4_sr_wan").options[0].selected = true;
		...
		{
			var curWan = getElement("v4_sr_wan_" + temp).innerHTML;
			...
			getElement("v4_sr_ip").value = getElement("v4_sr_ip_" + temp).innerHTML;			
			getElement("v4_sr_gw").value = getElement("v4_sr_gw_" + temp).innerHTML;			
			getElement("v4_sr_mask").value = getElement("v4_sr_mask_" + temp).innerHTML;
...
function clickRemoveAll(tabTitle)
{
	...
	document.location = "/goform/v4_static_route_delete_all";    
