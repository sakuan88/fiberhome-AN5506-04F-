...
function initValue()
{
	...
	if(ispNameCode == '21')
	{
		setDisplay("tr_auth_type", "");
		var get_auth_type = '<% getCfgGeneral(1, "tr069_auth_type"); %>';
		var auth_type = document.getElementById("tr069_auth_type");
		for(var i = 0; i< auth_type.length; i++)
		{	
			if(auth_type.options[i].value == get_auth_type)
			{
				auth_type.options[i].selected = true;
				break;
			}
		}	
	}
...
<form method=post name="fm_tr069_basic" action="/goform/setTR069Basic" onSubmit="return CheckValue()">
  <table id="tb_tr069Basic" class="tabal_bg" border="0" cellpadding="0" cellspacing="1" width="100%">
    <tbody>
      ...
      <tr id='tr_auth_type' style="display: none">
        <td class="tabal_left" id="ConnectionRequestAuthType_title">Connection Request AuthType</td>
        <td class="tabal_right"><select name="tr069_auth_type" id="tr069_auth_type" size="1">
            <option value="0" selected="selected">Digest</option>
